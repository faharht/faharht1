#!/usr/bin/env bun
/**
 * One-shot script: adds a `pl` field (Polish translation) to every sentence in
 * src/data/sentences/*.json that is missing one. Uses Lovable AI Gateway.
 *
 * Idempotent / resumable: rerunning only fills in sentences still missing `pl`.
 *
 * Run with: bun run scripts/translate_pl.ts
...
async function translateBatch(items: { en: string; ru: string }[]): Promise<string[]> {
  ...
  const prompt =
    "You translate English meanings of Russian sentences into natural Polish. " +
    "The Russian source is provided only as context. Translate the English meaning into Polish. " +
    ...
    '{"pl":["...","..."]} with exactly one Polish string per input, in order. No commentary.\n\n' +
    numbered;
  ...
  const parsed = JSON.parse(content) as { pl?: unknown };
  if (!Array.isArray(parsed.pl)) throw new Error("Bad response: no pl[]");
  const arr = parsed.pl as unknown[];
...
async function processFile(file: string) {
  ...
  const missingIdx = sentences.flatMap((s, i) => (s.pl && s.pl.trim() ? [] : [i]));

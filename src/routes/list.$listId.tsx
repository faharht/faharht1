...
function ListPage() {
  ...
  function playOne(idx: number) {
    ...
    speak(s.ru, {
      rate: settings.speed,
      onEnd: () => setCurrentIdx((c) => (c === idx ? null : c)),
      onError: () => setCurrentIdx(null),
    });
  }
    ...
    for (let i = 0; i < visibleSentences.length; i++) {
      ...
      for (let r = 0; r < settings.reps; r++) {
        ...
        await new Promise<void>((resolve) => {
          speak(s.ru, { rate: settings.speed, onEnd: resolve, onError: resolve });
        });
        if (cancelRef.current) break;
  ...
  const noVoiceWarning =
    typeof window !== "undefined" && hasSpeech() && window.speechSynthesis.getVoices().length === 0
      ? false // voices load async; we don't block UI
      : false;

  return (
    <div className="min-h-screen bg-[oklch(0.985_0.008_180)] pb-32">
      {/* Sticky header */}
      <header className={cn("sticky top-0 z-30 border-b border-border/60 bg-[oklch(0.96_0.025_180)]/95 backdrop-blur")}>
      ...
      <main className="mx-auto max-w-2xl px-4">
        ...
        <div className="mt-4 flex items-center gap-2 rounded-2xl border border-border/60 bg-card px-3 py-2 shadow-sm">
          <button
            ...
            className="grid h-11 w-11 place-items-center rounded-full bg-primary text-primary-foreground shadow-sm transition hover:opacity-90 disabled:opacity-40"
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <IconBtn label="Hints"><MessageCircle className="h-4 w-4" /></IconBtn>
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 rounded-lg border border-border bg-card px-2 py-1.5 text-sm text-foreground",
      ...
      {noVoiceWarning && (
        <div className="fixed inset-x-0 bottom-0 z-40 mx-auto max-w-2xl p-4">
          <div className="rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900">
            No Russian voice installed on this device. Install a Russian TTS voice to hear audio.
          </div>
        </div>
      )}
    </div>
...
function IconBtn({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={cn(
        "grid h-9 w-9 place-items-center rounded-lg border bg-card transition",
        active
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="grid h-8 w-8 place-items-center rounded-lg border border-primary/30 bg-primary/5 text-primary hover:bg-primary/10 disabled:opacity-40"
      ...
      {hideRu ? (
        ...
        <>
          <p
            className={cn("mt-3 font-semibold leading-snug text-foreground", textSizeClass)}
            lang="ru"
          >
            {sentence.ruStressed || sentence.ru}
          </p>
          {showTranslit && (
            <p className="mt-1 text-sm italic text-muted-foreground">{sentence.translit}</p>
          )}
        </>
      )}
    </li>
